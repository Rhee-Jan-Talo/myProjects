...
class salary_infoForm(forms.ModelForm):
         class Meta:
            model = salary_info
            fields = ['profile_id', 'salary_type','regular', 'overtime','bonus', 'tips','severance',
...
class salary_EditinfoForm(forms.ModelForm):
         class Meta:
            model = salary_info
            fields = ['salary_type','regular', 'overtime','bonus', 'tips','severance',
...
class add_scheduleForm(forms.ModelForm):
         class Meta:
            ...
            fields = ['date', 'time', 'etime', 'doctor_lastname']

class edit_scheduleForm(forms.ModelForm):
         class Meta:
            model = available_schedules
            fields = ['date', 'time', 'etime', 'doctor_lastname','availability']

class add_inventoryForm(forms.ModelForm):
         class Meta:
            model = item_inventory
            fields = ['itemname', 'barcode','itemcategory', 'location','item_fee', 'quantity','units', 'expiry_date','srp'] #, 'expiry_date'
...
class make_paymentForm(forms.ModelForm):
         class Meta:
            model = payment
            fields = ['appointment_id','total_service_fee','total_tools_fee', 'total_payment', 'payment_method', 'date_paid', 'discount']
...
class buy_prescriptionForm(forms.ModelForm):
         class Meta:
            ...
            fields = [ 'patient_id', 'item_id', 'quantity', 'units', 'purchase_date', 'discount']
...
class add_inventoryForm2(forms.ModelForm):
         class Meta:
            model = item_inventory
            fields = ['itemname', 'barcode', 'itemcategory', 'location','item_fee', 'quantity','units','srp']
