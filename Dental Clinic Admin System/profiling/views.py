...
def generate_pdf_income(request,date):#nagamit
        ...
        tools_used = tools_items_used.objects.filter(date_used__contains = date)
        tot_presc =  get_income_presc(date)
        tot_pay = get_income_payments(date)
        tot_tools = get_income_tools(date)
        tot_service = get_income_service(date)
        total_inc = tot_presc + tot_tools + tot_service 
        total_inc = float("%.2f" % total_inc)
        context = {'tot_tools':tot_tools,'tools_used':tools_used, 'tot_service':tot_service,'tot_presc':tot_presc,'tot_pay':tot_pay,'total_inc':total_inc,
                    'd':d,"date": date, 'payment_details':payment_details, 'presc_details':presc_details}
        pdf = render_to_pdf('income_breakdown_pdf.html',context)
        return HttpResponse(pdf, content_type='application/pdf')#rendering the template


def generate_pdf_profit(request,date):#nagamit
    d = getDatee(date)
    list1 = get_list(date)
    profit = 0
    item_details = item_inventory.objects.filter(id__in = list1)
    for item in item_details:
        item.all_quan = item.tot_quan + item.quantity
    for things in item_details:
        things.net_income = things.tot_quan * (things.item_fee - things.srp)
        profit = profit + things.net_income
    context = {'d':d,'profit':profit,'list1': list1, 'item_details': item_details}
    pdf = render_to_pdf('profit_breakdown_pdf.html',context)
    return HttpResponse(pdf, content_type='application/pdf')
...
def editform(request, profile_id, profiletype): 
        ...
        if profiletype == 'patient':                              #EDITING PATIENT
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    all_data_emp_salary.salary_type = form.cleaned_data.get('salary_type')
                ...
                if request.method == 'POST':
                    ...
                    if form.is_valid():
                        ...
                        etime = form.cleaned_data.get('etime')
                        doc = form.cleaned_data.get('doctor_lastname')
                        some_bool = check_schedEdit(date,time,etime,doc,profile_id)
                        if some_bool == 'yes':
                            conflict = check_conflict2(date,time,etime,doc,profile_id)
                            if conflict == False:
                                all_data_schedule.date = form.cleaned_data.get('date')
                                all_data_schedule.time = form.cleaned_data.get('time')
                                all_data_schedule.etime = form.cleaned_data.get('etime')
                                ...
                                messages.error(request, 'Schedule Conflict Occured')
                                return render(request, 'editform.html', context)
                ...
                if request.method == 'POST':
                    ...
                    if form.is_valid():
                        ...
                        if form.cleaned_data.get('quantity') <= 0:
                            ...
                            all_data_inventory.srp = form.cleaned_data.get('srp')
    ...
    #                    all_data_items_tools_used.item_tools_fee = form.cleaned_data.get('item_tools_fee')
                        ...
                        update_tot_quan_edit(before_edit, after_edit, int(profile_id))
...
def check_conflict2(date, time,etime,doc,profile_id):
    schedule_new = available_schedules.objects.get(pk = profile_id)
    some_id = schedule_new.id
    schedule_all = available_schedules.objects.exclude(pk = some_id).filter(availability = True)
    some_bool2 = False
    for things in schedule_all:
        if things.doctor_lastname.lower() == doc.lower():
            if things.date == date:
                if things.time <= time:
                    if things.etime >= time:
                        some_bool2 = True
...
def check_schedEdit(date, time,etime,doc,profile_id):
    schedule_new = available_schedules.objects.get(pk=profile_id)
    some_id = schedule_new.id
    schedule_all = available_schedules.objects.exclude(pk = some_id)
    for things in schedule_all: #existing sched
        if things.date == date and things.time == time and things.doctor_lastname == doc and things.etime == etime:
            return"exist"
        else:
            pass
    if date < date.today(): #passed date
        return 'date'

    elif date == date.today(): #today date
        timeS = datetime.now()
        timeS = timeS.strftime('%H:%M')
        if time.strftime("%H:%M") < timeS: #lampas ang time
            return 'time'
        else:
            if etime.strftime("%H:%M") < time.strftime("%H:%M"):
                return 'time'
            else:
                return 'yes'
...
def update_tot_quan_edit(before_edit, after_edit, profile_id):
    tools_data = tools_items_used.objects.get(pk = profile_id)
    item_key = tools_data.item_id.id
    item_details = item_inventory.objects.get(pk = item_key)
    item_details.tot_quan = int(item_details.tot_quan) - int(before_edit)
    item_details.tot_quan = int(item_details.tot_quan) + int(after_edit)
    item_details.tot_amount = float(item_details.tot_amount) - (int(before_edit) * float(item_details.item_fee))
    item_details.tot_amount = float(item_details.tot_amount) + float(tools_data.item_tools_fee)
    item_details.save()
...
def check_conflict():
    ...
    some_bool = True
    for things in schedule_all:
        if things.doctor_lastname.lower() == schedule_new.doctor_lastname.lower():
            if things.date == schedule_new.date:
                if things.time <= schedule_new.time:
                    if things.etime >= schedule_new.time:
                        some_bool = False
                        schedule_new.delete()
                    elif things.etime <= schedule_new.time:
                        pass #not conflict
                else:
                    pass
            else:
...
def check_schedule():
    ...
    schedule_all = available_schedules.objects.exclude(pk = some_id)
    for things in schedule_all: #existing sched
        if things.date == schedule_new.date and things.time == schedule_new.time and things.doctor_lastname == schedule_new.doctor_lastname and things.etime == schedule_new.etime:
            schedule_new.delete()
            return "exist" 
        else:
            pass
    if schedule_new.date < date.today(): #date passed
        schedule_new.delete()
        return "date"


    elif schedule_new.date == date.today(): #today ang date
        time = datetime.now()               #ang time not passed
        time = time.strftime('%H:%M')
        if schedule_new.time.strftime("%H:%M") < time:
            schedule_new.delete() #time has passed
            return 'time'
        else:
            if schedule_new.etime.strftime("%H:%M") <= schedule_new.time.strftime("%H:%M"):
                schedule_new.delete()
                return 'time'
            else:
                return 'yes'
...
def view_schedule(request):
    ...
    for things in sched_data:
        if things.date < date.today():
            ...
            pass
    all_data_schedule = available_schedules.objects.filter(availability = True).order_by('date')
    context = {'all_data_schedule':all_data_schedule}
...
def update_tot_quan():
    tools_data = tools_items_used.objects.last()
    item_id = tools_data.item_id.id
    item_data = item_inventory.objects.get(pk = item_id)
    item_data.tot_quan = int(item_data.tot_quan) + int(tools_data.quantity_used)
    item_data.tot_amount = float(item_data.tot_amount) + float(tools_data.item_tools_fee)
    item_data.save()
...
def make_payment(request, appointment_id, service_fee, procedures_fee, tools_fee, discount):
    if discount == 'None':
        discount = 0
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if procedures_fee == 'None':
                ...
                total_payment = float(total_payment) - (float(discount) * float(procedures_fee))
                discounted_service = float(procedures_fee)-(float(discount) * float(procedures_fee))
            elif tools_fee == 'None':
...
#               total_payment =float(procedures_fee) + float(tools_fee)
            ...
            context = {'discounted_service':discounted_service,'discount':discount,'pk':pk, 'all_data_appointments' : all_data_appointments, 'procedures_fee' : procedures_fee, 'tools_fee' : tools_fee, 'total_payment' : total_payment}   #, 'service_fee' : service_fee 
...
#            total_payment =float(procedures_fee) + float(tools_fee)
            ...
            total_payment = float(total_payment) - (float(discount) * float(procedures_fee))
            discounted_service = float(procedures_fee)-(float(discount) * float(procedures_fee))
        context = {'discounted_service':discounted_service,'discount':discount, 'pk':pk, 'all_data_appointments' : all_data_appointments, 'service_fee' : service_fee, 'procedures_fee' : procedures_fee, 'tools_fee' : tools_fee, 'total_payment' : total_payment}    
...
def calculate_payment(request, primary_key):
    ...
    discount = float(purchase_data.discount)
...
def update_item_info():
    presc = prescription_purchase.objects.last()
    item_id = presc.item_id.id
    item_info = item_inventory.objects.get(pk = item_id)
    item_info.tot_quan = int(item_info.tot_quan) + int(presc.quantity)
    item_info.tot_amount = float(presc.total_amount) + float(item_info.tot_amount)
    item_info.tot_amount ="%.2f"%item_info.tot_amount
    item_info.save()
    return
...
def view_appointment_income(request):
    ...
    list1 = []
    for things in payment_details: #no need to do this because nanay separate na tools fee ug service fee
        item_fee = get_item_fee(things.appointment_id)
        #things.total_payment = things.total_payment - item_fee  #UNREMOVE THIS
        list1.append(things)
    context = {'payment_details' : payment_details, "list1": list1}
    return render(request, 'view_appointment_income.html',context)



def get_item_fee(primary_key):
    items = tools_items_used.objects.filter(appointment_id = primary_key)
    item_fee = 0
    for i in items:
        item_fee = item_fee + i.item_tools_fee
    return item_fee
                ...
                if name != "":
                    ...
                    if service != "":
                        ...
                        if doc != "":
                            ...
                            context = {'payment_details' : payment_details,'newlist':newlist}
...
@login_required
def view_presc_income(request):
    item_details = item_inventory.objects.all()
    stockin = stock_in.objects.all()
    for item in item_details:
        for stock in stockin:
            if item.id == stock.item_id.id:
                item.total_spent = float(stock.total_payment)/int(item.quantity)
                item.net_income = float(item.tot_amount) - float(item.total_spent)
...
def view_presc_income(request):
    ...
    presc_id = prescription_purchase.objects.all()
    tools_id =  tools_items_used.objects.all()
    tools_id = tools_id.values_list('item_id', flat=True).distinct()
    presc_id = presc_id.values_list('item_id', flat=True).distinct()
    list1 = get_ids(tools_id, presc_id)
    tools_income = item_inventory.objects.filter(id__in = list1)


    context = {'list1':list1,'presc_id' : presc_id,'presc_details' : presc_details,
                'tools_id':tools_id, 'tools_income':tools_income,
              }
    return render(request, 'view_presc_income.html',context)


def get_total_sold(primary_key):
    items = tools_items_used.objects.filter(item_id = primary_key)
    tot_quan = items.aggregate(Sum('quantity_used'))
    return tot_quan

def get_total_presc(primary_key):
    items = prescription_purchase.objects.filter(item_id = primary_key)
    tot_quan = items.aggregate(Sum('quantity'))
    return tot_quan


'''

def get_ids(tools_id, presc_id):
    list1=[]
    for i in tools_id:
        list1.append(i)
    for i in presc_id:
...
def add_perishableList(request,itemname ,barcode,itemcategory,location,item_fee, quantity, units,expiry_date,srp):
    ...
    context = {'srp':srp,'itemname' : itemname, 'some_id' : some_id, 'quantity': quantity, 'units' : units,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get('quantity') <=0:
                ...
                if some_bool == True:
                    return redirect('add_perishableList',itemname ,barcode,itemcategory,location,item_fee, quantity, units,expiry_date,srp)
                else:
                    messages.error(request, 'Invalid Date')
                    return render(request,'add_inventory2.html', {})
        else:
...
def add_new_item(request, itemname ,barcode,itemcategory,location,item_fee, quantity, units,srp):
    item_data = item_inventory.objects.last()
    some_id = int(item_data.id) 
    items_data = item_inventory.objects.filter(expiry_date__isnull = False)
    context = {'srp':srp,'itemname' : itemname, 'some_id' : some_id, 'quantity': quantity, 'units' : units,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if quantity <= 0:
                ...
                #quantity = form.cleaned_data.get('quantity')
                #units = form.cleaned_data.get('units')
                return redirect('add_new_item',itemname ,barcode,itemcategory,location,item_fee, quantity, units, srp)
        else:
            messages.error(request, 'Please supply every information and follow the given format')
            return render(request,'add_inventory3.html', {})
    else:
...
def total_monthly_income(request):
    if request.method =="POST":
        ...
        if date !="":
            ...
            context = {'tot_service':tot_service,'total_inc':total_inc,'d':d,'date': date,'presc_details':presc_details,'payment_details':payment_details,'tot_presc':tot_presc,'tot_pay':tot_pay}
...
def total_monthly_profit(request):
    if request.method =="POST":
        ...
        if date !="":
            ...
            context = {'tot_service':tot_service, 'date': date,'d':d,'total_exp':total_exp,'total_inc':total_inc,'total_p':total_p }
...
def get_income_tools(date):
    tools = tools_items_used.objects.filter(date_used__contains = date)
    tot_tools = tools.aggregate(Sum('item_tools_fee'))
    if tot_tools["item_tools_fee__sum"] != None:
        tot_tools = float("%.2f" % tot_tools["item_tools_fee__sum"])
        return tot_tools
        
    else:
        return 0
'''
def get_income_service(date):
    payment_details = payment.objects.filter(date_paid__contains = date)
    tot_service = payment_details.aggregate(Sum('total_service_fee'))
    if tot_service["total_service_fee__sum"] != None:
        tot_service = float("%.2f" % tot_service["total_service_fee__sum"])
        return tot_service
...
def get_income_service(date):
    payment_details = payment.objects.filter(date_paid__contains = date)
    tot_pay = payment_details.aggregate(Sum('total_service_fee'))
    if tot_pay["total_service_fee__sum"] != None:
        tot_pay = float("%.2f" % tot_pay["total_service_fee__sum"])
    else:
        tot_pay= 0
...
def profit_breakdown(request, date):
    list1 = get_list(date)
    profit = 0
    item_details = item_inventory.objects.filter(id__in = list1)
    for item in item_details:
    ...
    for things in item_details:
        things.net_income = things.tot_quan * (things.item_fee - things.srp)
        profit = profit + things.net_income
    context = {'profit':profit,'list1': list1, 'item_details': item_details}
    return render(request,'profit_breakdown.html',context)
...
def get_list(date):
    list1 = []
    items_used = tools_items_used.objects.filter(date_used__contains = date)
    items_sold = prescription_purchase.objects.filter(purchase_date__contains = date)
    items_used = items_used.values_list('item_id', flat=True).distinct()
    items_sold = items_sold.values_list('item_id', flat=True).distinct()
    for i in items_used:
        list1.append(i)
    for i in items_sold:
        list1.append(i)
